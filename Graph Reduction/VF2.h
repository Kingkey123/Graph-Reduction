...
public:
	...
	{
		vector<prii> s; // Store matched pairs M1 和 M2
...
public:
	...
	int contract(Graph& QG, Graph& DG); //让DG即DBGraph中的图数据改变，用于下一次solve
	int contract2(Graph& DG);
	int vf2(const Graph &QG, const Graph& DG, const int &QID, const State &s, bool isOutput = 0, bool isContract = 0);       // Run VF2 on QueryGraph & DBGraph (engine)
...
private:
	...
	bool query(const State &s);                                   // Run VF2 on pat & g (main procedure)
